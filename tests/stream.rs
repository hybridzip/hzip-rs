...
fn test_write_file_without_model() {
    ...
    if !hz.file_exists("/test_write_file_without_model.txt".to_string()).unwrap() {
        hz.write_file(
            data,
            StreamConfig {
...
fn test_write_model() {
    ...
    let mut file =
        File::open("/home/supercmmetry/Projects/hzip-research/Models/victini.enwik9.hm").unwrap();
